...
public class PrepareData {
    ...
    public void trainNetwork(BasicNetwork network, MLDataSet training, int forwardStep) {
        ...
        do {
            ...
            progress.setProgress(initProgress + (int) (33 - ((train.getError() - error) * 33.0D)));
            progress.updateProgress("Training neural network (" + forwardStep + " timestamp forecast)...");
    ...
    private void prepare(TickerSymbol ticker, final Date from, final Date to, int forwardStep) {
        ...
        progress.setProgress(initProgress);
        progress.updateProgress("Loading data set...");
        ...
        progress.setProgress(initProgress + 5);
        progress.updateProgress("Calculating indicators for neural inputs...");
...
//            trend.setInputValue(((ema5.getValues()[j] > ema10.getValues()[j]) ? 1.5 : (ema5.getValues()[j] == ema10.getValues()[j]) ? 1.0 : 0.5));
        ...
        // Train the network
        trainNetwork(network, trainingSet, forwardStep);
