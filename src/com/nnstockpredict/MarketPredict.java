...
/*
 * Encog(tm) Java Examples v3.2 http://www.heatonresearch.com/encog/
...
public class MarketPredict {
    ...
    public static void main(String[] args) {
        if (args.length < 1) {
            System.out.println("MarketPredict [data dir] [symbol] [generate/train/prune/evaluate] [time]");
        } else {
            File dataDir = new File(args[0]);
            String symbol = args[1];
            if (args[2].equalsIgnoreCase("generate")) {
                MarketBuildTraining.generate(symbol, dataDir);
            } else if (args[2].equalsIgnoreCase("train")) {
                MarketTrain.train(dataDir);
            } else if (args[2].equalsIgnoreCase("evaluate")) {
                MarketEvaluate.evaluate(dataDir);
            } else if (args[2].equalsIgnoreCase("prune")) {
